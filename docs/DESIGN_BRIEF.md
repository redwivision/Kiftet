...
- **Oxide Rust** `#B54A2C` — one meaning, used consistently: something needs attention. Covers both "gap identified" in the diagnostic screen and "incorrect" in retest feedback — one of the two functional colours allowed in the monochrome identity, muted and mineral rather than generic red.
- **Sage** `#5C7A5E` — the other half of the feedback convention: solid, covered, correct. Used for "already solid" bars, covered concepts, retest-correct verdicts, and (as the moment it literally means) the central label on the "Gap closed." ring when the mark settles.
...
Rust and Sage double as the correct/incorrect feedback convention deliberately — using the same two colors for both the diagnostic gap-finding and the retest scoring keeps the whole app speaking one consistent visual language, rather than introducing a separate "quiz app" color system on top of the brand. Both values are **fixed, not per-room**: they follow the room's ground/candle only as much as legibility needs, so "gap / solid" never quietly changes meaning when a student switches rooms.
