...
1. **Chapter ingestion** — chapter text goes to the LLM with a prompt asking it to output a structured list of: (a) core testable concepts, (b) common misconceptions specific to that concept. Store as `ConceptNode` rows tied to the chapter. Do this once per chapter, cache the result — don't re-run extraction on every session. **For a student's own textbook (Phase 6):** the file (PDF up to 100 MB / pasted text) is opened on the device, the PDF's outline/TOC is read (pdf.js `getOutline()`), and the book is sliced into **chunks** — one TOC section per extraction call. Only cleaned raw text is POSTed to `/api/chapters/ingest`. File bytes never leave the phone.
