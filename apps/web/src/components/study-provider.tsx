...
function reducer(state: StudyState, action: StudyAction): StudyState {
  switch (action.type) {
    ...
    case "LESSON":
      return { ...state, phase: "lesson", lessonText: action.text, error: null, notice: null };
    case "QUESTIONS":
      return {
        ...state,
        phase: "retest",
...
export function StudyProvider({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const chapter = chapters.find((c) => c.id === session.chapterId) ?? null;
        if (!chapter) {
  ...
  const startRetest = useCallback(() => {
    ...
    return run(
      () =>
        api<{ questions: { question: string }[] }>(`/sessions/${sessionId}/retest`, {
          method: "POST",
          body: JSON.stringify({
            missing: gaps.missing,
            misconceptions: gaps.misconceptions,
          ...
          dispatch({ type: "QUESTIONS", questions: questions.map((q) => q.question) }),
        ),
      undefined,
    );
  }, [run, sessionId]);
  ...
  const retryLast = useCallback(() => {
    ...
    dispatch({ type: "BUSY", busy: true });
    return replay()
      .catch((err: unknown) => dispatch({ type: "ERROR", message: apiError(err) }))
  ...
  const setNotice = useCallback((message: string | null) => dispatch({ type: "NOTICE", message }), []);
  const clearNotice = useCallback(() => dispatch({ type: "NOTICE", message: null }), []);
  const clearError = useCallback(() => dispatch({ type: "ERROR", message: null }), []);
  const beginRecall = useCallback(() => dispatch({ type: "GO_PHASE", phase: "recall" }), []);
  const viewGaps = useCallback(() => dispatch({ type: "GO_PHASE", phase: "gaps" }), []);
