...
export function VoxideRing() {
  ...
  const [initReady, setInitReady] = useState(() => (client ? client.isInitialized : false));
  useEffect(() => {
  ...
  return (
    <div className="relative grid place-items-center" aria-live="polite">
      {initReady && isActive && <span className="absolute inset-0 animate-ping rounded-full bg-gold/20" />}
      <button
        type="button"
        onClick={onClick}
        aria-label={label}
        className={cn(
          ...
          initReady && (status === "idle") && "border-gold/40 text-gold/80 hover:border-gold hover:text-gold",
          ...
          initReady && isProcessing && "border-manuscript/30 text-manuscript/60",
